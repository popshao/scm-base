...
public class CloneUtil {
    ...
    private static Log log = LogFactory.getLog(CloneUtil.class);

    /**
     * 无需进行复制的特殊类型数组
     */
    private static Class[] needlessCloneClasses = new Class[] { String.class, Boolean.class, Character.class, Byte.class,
    ...
    private static Object createObject(Object value){
        try {
            ...
            log.error("can not instance object:" + value.getClass().getSimpleName());
        } catch (IllegalAccessException e) {
            log.error("object is not a public class :" + value.getClass().getSimpleName());
        }
        return null;
    }
    /**
    ...
    public static Object clone(Object value, int level) {
        ...
        if (value instanceof Collection){// 复制新的集合
            Collection tmp = null;
            try {
                tmp = (Collection) c.newInstance();
            } catch (InstantiationException e) {
                log.error("can not instance object:" + c.getSimpleName());
                throw new RuntimeException(e);
            } catch (IllegalAccessException e) {
                log.error("object is not a public class:" + c.getSimpleName());
                throw new RuntimeException(e);
            }
